var Gallery = {
  ...
  watch:{
      //watch for changes
    isLoaded: function(){//when page loads
        ...
        <template v-for="gallery in exhibit">
            ...
            <div class="containter p-3">
                ...
                <ul class="nav nav-pills nav-fill">
                    ...
                    <button type="button" class="btn btn-secondary nav-link" @click="filterArtist('all')">All</button>
                    </li>
                    <li class="nav-item mb-2" v-for="artist in gallery.artists.artist"><button type="button" class="btn btn-secondary nav-link" @click="filterArtist(artist.attributes.name)" >{{artist.attributes.name}}</button></li>
                </ul>
            </div>
          ...
          itemtype="http://schema.org/ImageGallery">
                ...
                <div class="list row">
                    ...
                    <template v-for="image in gallery.installation.image" >
                        ...
                        <figure class="col-3 col-md-2 mb-3 p-2" v-for="src in image.source" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                        <a :href="'/data/'+ src" itemprop="contentUrl" data-size="600x400">
                        <img :src="'/data/'+ src " class="img-thumbnail " itemprop="thumbnail">
                        </a>
                        
                        <figcaption v-for="c in image.caption" :itemprop="c" hidden>{{c}}</figcaption>
                    ...
                    <template v-for="artist in gallery.artists.artist" >
                        <template v-if="artistFilter == artist.attributes.name">
                            <template v-for="image in artist.image">  
                                <figure class="col-3 col-md-2 mb-3 p-2" v-for="src in image.source" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                                    <a :href="'/data/'+ src" itemprop="contentUrl" data-size="600x400">
                                    <img :src="'/data/'+ src " class="img-thumbnail">
                                    </a>
                                <figcaption v-for="c in image.caption" :itemprop="c" hidden v-html=c>{{c}}</figcaption>
                                </figure>
                            </template>
                        </template>
                        <template v-if="artistFilter == 'all'">
